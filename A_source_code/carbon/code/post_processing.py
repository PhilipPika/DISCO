...
def calc_carb_dat(conc_DIC, conc_ALK, T):
    ...
    with suppress_stdout_stderr():
        carb_dat = np.dstack(mocsy.mvars(temp=T, sal=0, alk=conc_ALK/12, \
                                      ...
                                      optp='db', optb="u74", optk1k2='l', \
                                      optkf="dg", optgas='Pinsitu'))[0][0]
...
def calc_ph_pco2(params,dformat):
    print('POST PROCESSING pH/pCO2 DATA IN '+params.outputdir+' AS '+dformat)
    # start timer
    ...
    if dformat=='NETCDF':
        ...
        Tlist = sorted(directory.get_files_with_str(os.path.join(\
                    params.outputdir, '..', "STREAM_ENV_CONDITIONS", "subgrid"), '*temperature*'))

        icell_list = ascraster.create_mask(params.file_mask, params.maskid, \
                                           params.mask_bool_operator,numtype=int)
        dum_asc = ascraster.Asciigrid(ascii_file=params.file_mask)
        mask_2d = make_mask.do(params.file_mask, params.maskid, dum_asc, \
                               logical=params.mask_bool_operator, mask_type='np_grid')
